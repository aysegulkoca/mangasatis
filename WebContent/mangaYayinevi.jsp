...
<% 
String id = request.getParameter("yayinevi");

	int yid = Integer.parseInt(id);
	ArrayList<Manga> mangalar = new ArrayList<Manga>(MangaDAO.mangalariYayinEvineGoreListele(yid));
	            ...
	            <% for(int i = 0 ; i < mangalar.size(); i++) { %>
	                <div class="card border-warning" style="width: 18rem;">
	                    <div class="card-body">
	                        ...
	                        <h6 class="card-subtitle mb-2 text-muted">Türü: <%= MangaDAO.mangaTurunuGetir(mangalar.get(i).getTur_turId()) %></h6>
	                        <a href="manga?mangaId=<%=mangalar.get(i).getMangaId() %>" class="btn btn-outline-success"><i class="icon fas fa-eye"></i>Mangaya Git</a>
	                    </div>
	                </div>
	            <%} %>
	            </div>
